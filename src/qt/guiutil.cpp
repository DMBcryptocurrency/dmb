...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Digitalmoneybits"))
        return false;
...
{
    // Convert Digitalmoneybits:// to Digitalmoneybits:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Digitalmoneybits://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Digitalmoneybits:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Digitalmoneybits.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Digitalmoneybits.lnk
...
{
    return GetAutostartDir() / "Digitalmoneybits.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Digitalmoneybits.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Digitalmoneybits\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Digitalmoneybits app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Digitalmoneybits app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Digitalmoneybits-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Digitalmoneybits-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Digitalmoneybits-Qt"));
