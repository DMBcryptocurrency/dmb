...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDigitalmoneybitsAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDigitalmoneybitsAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDigitalmoneybitsAmount = strAnonymizeDigitalmoneybitsAmount.remove(strAnonymizeDigitalmoneybitsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigitalmoneybitsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDigitalmoneybitsAmount*COIN) nMaxToAnonymize = nAnonymizeDigitalmoneybitsAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDigitalmoneybitsAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDigitalmoneybitsAmount));
        strAnonymizeDigitalmoneybitsAmount = strAnonymizeDigitalmoneybitsAmount.remove(strAnonymizeDigitalmoneybitsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigitalmoneybitsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeDigitalmoneybitsAmount == 0){
