...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDigitalmoneybitsAmount"))
        settings.setValue("nAnonymizeDigitalmoneybitsAmount", 1000);
    nAnonymizeDigitalmoneybitsAmount = settings.value("nAnonymizeDigitalmoneybitsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDigitalmoneybitsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeDigitalmoneybitsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDigitalmoneybitsAmount:
            return QVariant(nAnonymizeDigitalmoneybitsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeDigitalmoneybitsAmount:
            nAnonymizeDigitalmoneybitsAmount = value.toInt();
            settings.setValue("nAnonymizeDigitalmoneybitsAmount", nAnonymizeDigitalmoneybitsAmount);
            emit AnonymizeDigitalmoneybitsAmountChanged(nAnonymizeDigitalmoneybitsAmount);
            break;
        default:
            break;
        }
