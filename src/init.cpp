...
void Shutdown()
{
    ...
    RenameThread("Digitalmoneybits-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Digitalmoneybits.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Digitalmoneybitsd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeDigitalmoneybitsamount=<n> " + _("Keep N Digitalmoneybits anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Digitalmoneybits is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Digitalmoneybits is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Digitalmoneybits version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Digitalmoneybits server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Digitalmoneybits") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Digitalmoneybits to complete") << "\n";
    ...
    nAnonymizeDigitalmoneybitsAmount = GetArg("-anonymizeDigitalmoneybitsamount", 0);
    if(nAnonymizeDigitalmoneybitsAmount > 999999) nAnonymizeDigitalmoneybitsAmount = 999999;
    if(nAnonymizeDigitalmoneybitsAmount < 2) nAnonymizeDigitalmoneybitsAmount = 2;
    ...
    LogPrintf("Anonymize Digitalmoneybits Amount %d\n", nAnonymizeDigitalmoneybitsAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1DMB+1000 == (.1DMB+100)*10
       10DMB+10000 == (1DMB+1000)*10
    */
